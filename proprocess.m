function proprocess()
name = 'AMBench_625_Build1_Layer369';
load([name '.mat']);

transform(Layer.RadiantTemp,name);
end
function transform(tensor,name)
[n1,n2,tottime] = size(tensor);
tottime
patchx = 100;
patchy = 200;
k=10;
...
%          return
%      end
    themat = double(tensor(:,:,t));
    extendedmat = zeros(n1+2*patchx,n2+2*patchy)+0.0;
    %size(extendedmat(patch:(n1+patch-1),patch:(n2+patch-1)))
    %size(themat)
    %size(themat+extendedmat(patch:(n1+patch-1),patch:(n2+patch-1)))
    extendedmat(patchx:(n1+patchx-1),patchy:(n2+patchy-1)) = extendedmat(patchx:(n1+patchx-1),patchy:(n2+patchy-1))+themat;
    [pos,val]=topk(extendedmat,k);
    %mean(val)
    sumvec(t,1)=sum(sum(themat));
    if sum(sum(themat)) < 1e6
         %'should continue'
         continue
    end
    totfig = totfig+1;
    %val
    %pos
    avgpos = floor(mean(pos,1));
    %avgpos
    %avgpos
    
    extracted = extendedmat((avgpos(1)-hpx+1):(avgpos(1)+hpy-1),(avgpos(2)-hpy+1):(avgpos(2)+hpy-1));
    col_mean = mean(extracted);
    col_mean_sq = col_mean.^2;
    nrows = length(col_mean);
    meanpos = (1:nrows)*col_mean'/sum(col_mean);
    sqmeanpos = (1:nrows)*col_mean_sq'/sum(col_mean_sq);
    if meanpos<sqmeanpos
        extracted = extracted(:,nrows:-1:1);
    end
    %max(extracted)
    %figure
    %clf
    ...
    %HeatMap(extendedmat)
    %return
    Path=['cutplot/' name '_' num2str(t) '.pkl'];
    %saveas(gcf,Path);
    mat2np(extracted, Path, 'float64');
...
function [pos,val]=topk(sm,k)
[n1,n2] = size(sm);
data = reshape(sm,[n1*n2,1]);
[~,idx] = sort(data(:),'descend');
val = data(idx(1:k));
%pos = [1+floor(idx/n2), 1+mod(idx,n2)];
pos = [1+mod(idx(1:k), n1), 1+floor(idx(1:k)/n1)];
end
